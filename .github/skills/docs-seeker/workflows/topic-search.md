...
# - 1-3 URLs: Read directly with `fetch_webpage` tool
# - 4-5 URLs: Spawn 2-3 `Explore` subagents in parallel via `runSubagent`
...
# Returns: 2-3 date-specific URLs (writes ./llms.txt in cwd)

# Read URLs directly with `fetch_webpage` tool
...
# Process URLs via 2 `Explore` subagents in parallel
