...
allowed-tools: [read, search, web, execute]
metadata:
  version: "3.1.0"
---
...
#    PowerShell: Get-Content llms.txt | node scripts/analyze-llms-txt.js -
#    POSIX:      cat llms.txt | node scripts/analyze-llms-txt.js -
```
...
node scripts/fetch-docs.js "<query>"    # → 2-3 URLs (writes ./llms.txt)
# Read URLs with `fetch_webpage` tool
```
...
Get-Content llms.txt | node scripts/analyze-llms-txt.js -  # → {totalUrls, distribution}
# Spawn `Explore` subagents per recommendation (via `runSubagent`)
```

> **Requirements:** Node.js ≥14 + outbound HTTPS to `context7.com`.
