...
1. **Search Strategy**:
   ...
   - If `gemini` bash command is not available, fallback to `fetch_webpage` tool.
   - Run multiple `gemini` bash commands or `fetch_webpage` tools in parallel to search for relevant information.
