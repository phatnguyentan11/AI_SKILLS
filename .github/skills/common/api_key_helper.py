...
def find_api_key(skill_dir: Optional[Path] = None) -> Optional[str]:
    ...
    github_env = project_dir / '.github' / '.env'
    if github_env.exists():
        api_key = load_env_file(github_env)
        if api_key:
            print(f"✓ Using API key from {github_env}", file=sys.stderr)
            return api_key

    # Step 4: Check ./.github/skills/.env
    github_skills_env = project_dir / '.github' / 'skills' / '.env'
    if github_skills_env.exists():
        api_key = load_env_file(github_skills_env)
        if api_key:
            print(f"✓ Using API key from {github_skills_env}", file=sys.stderr)
            return api_key
