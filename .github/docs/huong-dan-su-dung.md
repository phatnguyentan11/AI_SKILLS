...
| Prompt files | 17 | Workflow gọi bằng `/tên` |
| Custom agents | 8 | Personas gọi bằng `@tên` |
| Agent skills | 15 | Domain knowledge tự động |
| Workflow | 1 | Governance checks thủ công |
| Docs base | 3 | Tài liệu dự án cập nhật liên tục |
...
|---|---|---|
| `/ask` | Hỏi câu hỏi kỹ thuật có context repo | `/ask tại sao PaymentService dùng Unit of Work?` |
...
| `/banking-plan` | + Risk matrix, rollback, approval gates, verification commands | Mọi thay đổi ảnh hưởng DB, auth, payment, PII |
...
| **Security Reviewer** | Banking security, privacy, data integrity, audit | Thay đổi auth, payment, PII, DB |
...
│
├── prompts/                             ← 17 prompts, gọi bằng /tên-prompt
│   ├── ask.prompt.md
...
│
├── agents/                              ← 8 agents, chọn trong Agent mode
...
dotnet format MyProject.sln --include src/Services/PaymentService.cs --no-restore

# Nếu nhiều file:
dotnet format MyProject.sln --include src/Services/PaymentService.cs src/Controllers/AccountController.cs --no-restore
```
